...
def C_of_sigma(sigma, theta_b, theta_f):
    ...
    assert theta_f <= 0.0 and theta_f <= 1.0
    A = (1 - theta_b)(np.sinh(sigma * theta_f) / np.sinh(theta_f))
    B_1 = np.tanh(theta_f * (sigma + 0.5)) - np.tanh(theta_f / 2.0)
    B = theta_b * (B_1 / (2.0 * np.tanh(theta_f / 2.0)))
    return A + B
...
class Vgrid(ABC):
    ...
    def open(path):
        ...
        """
        with open(path) as f:
            return VgridTypeDispatch[
                VgridType(int(f.read().strip().split()[0])).name
            ].value.open(path)
...
class LSC2(Vgrid):
    ...
    def calc_m_grid(self):
        ...
        if self.m_grid:
            ...
            z_mas = np.ones([self.nhm, self.nvrt]) * np.nan
            eta = 0.0
            for m, [hsmi, nvi] in enumerate(zip(self.hsm, self.nv)):
                ...
                for k in np.arange(nvi):
                    ...
                    # compute zcoordinate
                    cs = (1 - self.theta_b) * np.sinh(self.theta_f * sigma) / np.sinh(
                        self.theta_f
                    ) + self.theta_b * (
                        np.tanh(self.theta_f * (sigma + 0.5))
                        - np.tanh(self.theta_f * 0.5)
                    ) / 2 / np.tanh(
                        self.theta_f * 0.5
                    )
                ...
                # normalize z_mas
                z_mas[m] = (
                    -(z_mas[m] - z_mas[m, 0]) * hsmi / (z_mas[m, nvi - 1] - z_mas[m, 0])
                )
    ...
    def make_m_plot(self):
        ...
        for i in np.arange(self.nhm - 1):
            if min(self.m_grid[i, : self.nv[i]] - self.m_grid[i + 1, : self.nv[i]]) < 0:
                print(
                    "check: master grid layer={}, hsm={}, nv={}".format(
                        i + 1, self.hsm[i + 1], self.nv[i + 1]
                    )
                )
        ...
        for i in np.arange(self.nhm):
            plot(i * np.ones(self.nvrt), self.m_grid[i], "k-", lw=0.3)
        for k in np.arange(self.nvrt):
            plot(np.arange(self.nhm), self.m_grid.T[k], "k-", lw=0.3)
    ...
    def calc_lsc2_att(self, gr3, crs=None):
        ...
        z_mas = np.array(
            [
                np.nan_to_num(z_mas_arr, nan=z_mas_arr[last_non_nan[i]])
                for i, z_mas_arr in enumerate(self.m_grid)
            ]
        )
        znd = z_mas[ind1] * (1 - rat[:, None]) + z_mas[ind2] * rat[:, None]
        # z coordinate
...
class SZ(Vgrid):
    ...
    def open(cls, path):
        ...
        irec = irec + 2
        nsigma = nvrt - kz + 1
        h_c, theta_b, theta_f = np.array(lines[irec].strip().split()[:3]).astype(
            "float"
        )
    ...
    def default(cls):
        ...
        return cls(1.0e6, [-1.0e6], 40.0, 1.0, 1.0e-4, [-1, 0.0])
        # return cls(h_s, ztot, h_c, theta_b, theta_f, sigma)
