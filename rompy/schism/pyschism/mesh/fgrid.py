...
class FrictionFilename(Enum):
    ...
    def _missing_(self, name):
        raise ValueError(f"{name} is not a valid filename for a friction " "file.")
...
class Fgrid(Gr3):
    ...
    def open(cls, file: Union[str, os.PathLike], crs: Union[str, CRS] = None):
        ...
        if cls.__name__ == "Fgrid":
            return FrictionDispatch[FrictionFilename(filename).name].value(
    ...
    def modify_by_region(self, hgrid, fname, value, depth1, flag):
        ...
        # if isinstance(region, Polygon):
        #    region = [region]
        gdf1 = gpd.GeoDataFrame({"geometry": [poly]}, crs=self.crs)
...
class ManningsN(Fgrid):
    ...
    def linear_with_depth(
        ...
        min_depth = np.min(-hgrid.values) if min_depth is None else float(min_depth)
        max_depth = np.max(-hgrid.values) if max_depth is None else float(max_depth)

        values = min_value + (-hgrid.values - min_depth) * (max_value - min_value) / (
            max_depth - min_depth
        )
...
class DragCoefficient(Fgrid):
    ...
    def linear_with_depth(
        ...
        values = bfric_river + (depth1 + hgrid.values) * (bfric_land - bfric_river) / (
            depth1 - depth2
        )
