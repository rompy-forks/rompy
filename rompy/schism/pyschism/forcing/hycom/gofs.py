...
from .base import Hycom, HycomComponent
...
class GofsDatasetCollection(ABC):
    ...
    def __init__(
        ...
        self.end_date = (
            self.maximum_end_date - self.sampling_interval
            if end_date is None
            else end_date
        )
        self.output_interval = (
            self.sampling_interval if output_interval is None else output_interval
        )
    ...
    def sampling_frequency(self):
        return 1.0 / self.sampling_interval.total_seconds()
...
class GofsForecastDatasets(GofsDatasetCollection):
    ...
    catalog_url = (
        "https://tds.hycom.org/thredds/catalog/GLBy0.08/expt_93.0/FMRC/runs/catalog.xml"
    )
        ...
        for required_date in self.required_datevector:
            for i, datevector in enumerate(self.datevectors):
                if required_date in datevector:
                    opendap_url = (
                        self.base_url
                        + self.xmlcatalog["catalog"]["dataset"]["dataset"][i][
                            "@urlPath"
                        ]
                    )
                    datasets.setdefault(required_date, Dataset(opendap_url))
                    break
        return datasets
    ...
    def datevectors(self):
        for file in self.xmlcatalog["catalog"]["dataset"]["dataset"]:
            ...
            end = datetime.strptime(file["timeCoverage"]["end"], "%Y-%m-%dT%H:%M:%SZ")
            steps = int((end - start).total_seconds() * self.sampling_frequency) + 1
            datevec = [
                start + x * timedelta(seconds=1 / self.sampling_frequency)
    ...
    def maximum_datetime(self):
        ...
        for rec in self.xmlcatalog["catalog"]["dataset"]["dataset"]:
            end_dates.append(
                dates.localize_datetime(
                    datetime.strptime(rec["timeCoverage"]["end"], "%Y-%m-%dT%H:%M:%SZ")
                )
            )
...
class GofsHindcastDatasets(GofsDatasetCollection):
    ...
    def datasets(self):
        ...
        for required_date in self.required_datevector:
            ...
            opendap_url = (
                f"https://tds.hycom.org/thredds/dodsC/{database}?lat[0:-1],lon[0:-1],"
    ...
    def pad_datasets(self, datasets):
        for dataset in datasets.values():
            if dataset is None:
                raise NotImplementedError("Hindcast data fetching is unavailable.")
...
class GOFSElevation(GOFSComponent):
    ...
    def put_boundary_ncdata(
        ...
        for i, (time, dataset) in enumerate(
            self.get_datasets(start_date, run_days, output_interval).items()
        ):
            if start_date.strftime("%Y-%m-%d") < datetime.now().strftime("%Y-%m-%d"):
                ds_base_date = datetime.strptime(
                    "".join(dataset["time"].units.split()[2:]), "%Y-%m-%d%H:%M:%S"
                )
                # time1 = dataset['time']
                ...
                ds_base_date = datetime.strptime(
                    "".join(dataset["time"].units.split()[2:-1]), "%Y-%m-%d%H:%M:%S.%f"
                )
            ds_timevector = [
                ds_base_date + timedelta(hours=x) for x in dataset["time"][:]
            ]
            requested_date = dates.nearest_cycle(
                start_date + i * output_interval, period=3
            ).replace(tzinfo=None)
            ...
            bbox = self._modified_bbox(dataset, Bbox.from_extents(*bounds))
            lon_idxs, lat_idxs = self._modified_bbox_indexes(
                bbox, dataset, pixel_buffer
            )
            ...
            print(
                f"the shape of zq is {len(zq)}, max zq is {np.max(zq)}, min zq is {np.min(zq)}"
            )
...
class GOFSVelocity(GOFSComponent):
    ...
    def put_boundary_ncdata(
        ...
        for i, (time, dataset) in enumerate(
            ...
            bzyx = np.c_[
                bz.reshape(np.size(bz)),
                by.reshape(np.size(by)),
                bx.reshape(np.size(bx)),
            ]
            zi = dataset["depth"][z_idxs]

            # First try with RegularGridInterpolator
            ui_fd = RegularGridInterpolator(
                (zi, yi, xi), ui, method="linear", bounds_error=False, fill_value=np.nan
            )
            u_interp = ui_fd(bzyx)
            ...
            if np.any(np.isnan(u_interp)):
                raise ValueError(
                    "No boundary  u velocity data for GOFS. "
                    "Try increasing pixel_buffer argument."
                )

            vi_fd = RegularGridInterpolator(
                (zi, yi, xi), vi, method="linear", bounds_error=False, fill_value=np.nan
            )
            ...
            if np.any(np.isnan(v_interp)):
                raise ValueError(
                    "No boundary data for GOFS. Try increasing pixel_buffer argument."
                )

            dst["time_series"][i, offset : offset + bz.shape[0], :, 0] = (
                u_interp.reshape(bz.shape)
            )
            dst["time_series"][i, offset : offset + bz.shape[0], :, 1] = (
                v_interp.reshape(bz.shape)
            )
    ...
    def put_boundary_ncdata(
        ...
        for i, (time, dataset) in enumerate(
            ...
            # the boundary and the data partially intersect
                ...
                ptemp_fd = NearestNDInterpolator(
                    bzyx[ptemp_idxs], ptemp_interp[ptemp_idxs]
                )
                ptemp_idxs = np.where(np.isnan(ptemp_interp))
            ...
            if np.any(np.isnan(ptemp_interp)):
                raise ValueError(
                    "No boundary  temperature data for GOFS. "
                    "Try increasing pixel_buffer argument."
                )
            dst["time_series"][i, offset : offset + bz.shape[0], :, :] = (
                ptemp_interp.reshape(bz.shape)
            )
    ...
    def put_boundary_ncdata(
        ...
        for i, (time, dataset) in enumerate(
            ...
            if np.any(np.isnan(salt_interp)):
                raise ValueError(
                    "No boundary  salt data for GOFS. "
                    "Try increasing pixel_buffer argument."
                )
            dst["time_series"][i, offset : offset + bz.shape[0], :, :] = (
                salt_interp.reshape(bz.shape)
            )
