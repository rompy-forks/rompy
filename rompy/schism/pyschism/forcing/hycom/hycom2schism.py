...
from ...mesh.base import Elements, Nodes
from ...mesh.vgrid import Vgrid
...
def interp_to_points_3d(dep, y2, x2, bxyz, val):
    ...
    if np.sum(idxs) != 0:
        val_int[idxs] = sp.interpolate.griddata(
            bxyz[~idxs, :], val_int[~idxs], bxyz[idxs, :], "nearest"
        )
    idxs = np.isnan(val_int)
    if np.sum(idxs) != 0:
...
def interp_to_points_2d(y2, x2, bxy, val):
    ...
    val_fd = sp.interpolate.RegularGridInterpolator(
        (y2, x2), np.squeeze(val), "linear", bounds_error=False, fill_value=float("nan")
    )
    val_int = val_fd(bxy)
    idxs = np.isnan(val_int)
    if np.sum(idxs) != 0:
        val_int[idxs] = sp.interpolate.griddata(
            bxy[~idxs, :], val_int[~idxs], bxy[idxs, :], "nearest"
        )
    idxs = np.isnan(val_int)
    if np.sum(idxs) != 0:
        logger.info(f"There is still missing value for {val}")
...
class OpenBoundaryInventory:
    ...
    def fetch_data(
        ...
        if elev2D and restart == False:
            ...
            dst_elev.createVariable(
                "time_series", "f", ("time", "nOpenBndNodes", "nLevels", "nComponents")
            )
        ...
        for it1, date in enumerate(timevector):
            ...
            for ibnd in ocean_bnd_ids:
                ...
                if TS or UV:
                    ...
                    y2i = np.tile(yi, [nvrt, 1]).T
                    bxyz = np.c_[
                        zcor2.reshape(np.size(zcor2)),
                        y2i.reshape(np.size(y2i)),
                        x2i.reshape(np.size(x2i)),
                    ]
                ...
                if date >= datetime.utcnow():
                    ...
                    url = (
                        f"https://tds.hycom.org/thredds/dodsC/{database}?lat[{lat_idx1}:1:{lat_idx2}],"
                ...
                if elev2D:
                    ...
                    dst_elev["time"][it] = it * 24 * 3600.0
                    if adjust2D:
                        elev_adjust = np.interp(blat, lats, msl_shifts)
                        dst_elev["time_series"][it, ind1:ind2, 0, 0] = (
                            ssh_int + elev_adjust
                        )
                    else:
...
class Nudge:
    ...
    def gen_nudge(self, outdir: Union[str, os.PathLike], rlmax=1.5, rnu_day=0.25):
        ...
        for i in self.ocean_bnd_ids:
            ...
            dis = abs(
                (lon + 1j * lat)[:, None]
                - (lon[bnd_idxs] + 1j * lat[bnd_idxs])[None, :]
            ).min(axis=1)
    ...
    def fetch_data(
        ...
        logger.info(
            f"Max relax distance is {rlmax} degree, max relax strengh is {rnu_day} days."
        )
        ...
        if restart:
            ...
            dst_temp.createVariable(
                "tracer_concentration", "f", ("time", "node", "nLevels", "one")
            )
        ...
        for it1, date in enumerate(timevector):
            ...
            for ibnd in self.ocean_bnd_ids:
                ...
                time_idx, lon_idx1, lon_idx2, lat_idx1, lat_idx2, x2, y2, _ = get_idxs(
                    date, database, bbox, lonc=nlonc, latc=nlatc
                )
...
class DownloadHycom:
    ...
    def fetch_data(
        ...
        for i, date in enumerate(timevector):
            ...
            time_idx, lon_idx1, lon_idx2, lat_idx1, lat_idx2, x2, y2, isLonSame = (
                get_idxs(date, database, self.bbox)
            )
