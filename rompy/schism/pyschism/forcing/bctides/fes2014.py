...
class FES2014(TidalDataProvider):
    ...
    def get_elevation(self, constituent, vertices):
        logger.info("Querying FES2014 for elevation constituent " f"{constituent}.")
        amp = self._get_interpolation("elevation", "amplitude", constituent, vertices)
        phase = self._get_interpolation("elevation", "phase", constituent, vertices)
        return amp, phase

    def get_velocity(self, constituent, vertices):
        logger.info("Querying FES2014 for velocity constituent " f"{constituent}.")
        uamp = self._get_interpolation("eastward_vel", "Ua", constituent, vertices)
        uphase = self._get_interpolation("eastward_vel", "Ug", constituent, vertices)
        vamp = self._get_interpolation("northward_vel", "Va", constituent, vertices)
        vphase = self._get_interpolation("northward_vel", "Vg", constituent, vertices)
    ...
    def constituents(self):
        ...
        if not hasattr(self, "_constituents"):
            files = glob.glob(
                appdirs.user_data_dir("fes2014") + "/ocean_tide_extrapolated/*.nc"
            )
    ...
    def _get_interpolation(self, phys_var, ncvar, constituent, vertices):
        ...
        # get interp index
        xi = np.asarray([x + 360.0 if x < 0.0 else x for x in vertices[:, 0]]).flatten()
        ...
        if ncvar == "phase" or ncvar == "Ug" or ncvar == "Vg":
            ...
            zi[mask] = zi[mask] + 360
        v0 = np.c_[
            zi[idy, idx], zi[idy, idx + 1], zi[idy + 1, idx], zi[idy + 1, idx + 1]
        ].T
        vmax = v0.max(axis=0)
        vmin = v0.min(axis=0)
        # handle phase jump
