...
from .base import TidalDataProvider
    ...
    raise FileNotFoundError(
        "\n".join(
            [
                ...
                f"2) set the environment variable `{fname}` to point" " to the file",
            ]
        )
    )
...
class TPXO(TidalDataProvider):
    ...
    def get_elevation(self, constituent, vertices):
        logger.info("Querying TPXO for elevation constituent " f"{constituent}.")
        amp = self._get_interpolation("elevation", "ha", constituent, vertices)
        phase = self._get_interpolation("elevation", "hp", constituent, vertices)
        return amp, phase

    def get_velocity(self, constituent, vertices):
        logger.info("Querying TPXO for velocity constituent " f"{constituent}.")
        uamp = self._get_interpolation("velocity", "ua", constituent, vertices) / 100.0
        uphase = self._get_interpolation("velocity", "up", constituent, vertices)
        vamp = self._get_interpolation("velocity", "va", constituent, vertices) / 100.0
        vphase = self._get_interpolation("velocity", "vp", constituent, vertices)
    ...
    def h(self):
        if not hasattr(self, "_h"):
            if self._h_file is None:
                ...
                if self._h_file is None:
                    self._h_file = (
                        pathlib.Path(appdirs.user_data_dir("tpxo")) / TPXO_ELEVATION
                    )
    ...
    def uv(self):
        if not hasattr(self, "_uv"):
            if self._u_file is None:
                ...
                if self._u_file is None:
                    self._u_file = (
                        pathlib.Path(appdirs.user_data_dir("tpxo")) / TPXO_VELOCITY
                    )
    ...
    def _get_interpolation(self, phys_var, ncvar, constituent, vertices):
        ...
        zi = ncarray[ncvar][lower_c.index(constituent.lower()), :, :]
        xo = np.asarray([x + 360.0 if x < 0.0 else x for x in vertices[:, 0]]).flatten()
