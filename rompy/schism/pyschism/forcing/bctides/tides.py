...
class Tides:
    ...
    def __call__(
        self, start_date: datetime, rnday: Union[int, datetime], constituent: str
    ):
        ...
        return (
            self.get_tidal_species_type(constituent),
            ...
            self.get_greenwich_factor(start_date, rnday, constituent),
        )  # FACE* # noqa:E501

    def get_elevation(self, constituent, vertices):

        if constituent.lower() == "z0":
            return np.full((vertices.shape[0],), float(self._Z0)), np.full(
                (vertices.shape[0],), 0.0
            )
    ...
    def get_velocity(self, constituent, vertices):
        if constituent.lower() == "z0":
            return tuple(4 * [np.full((vertices.shape[0],), 0.0)])
    ...
    def drop_constituent(self, constituent):
        if constituent not in self.active_constituents:
            raise ValueError(
                "Argument constituent must be one of " f"{self.active_constituents}"
            )
    ...
    def get_active_forcing_constituents(self):
        return [
            const
            for const, _ in self.active_constituents.items()
    ...
    def set_Z0(self, Z0):
        self._active_constituents["Z0"] = {"potential": False, "forcing": True}
        self._Z0 = Z0
    ...
    @_manage_dates
    def get_nodal_factor(
        self, start_date: datetime, rnday: Union[float, timedelta], constituent: str
    ):
    ...
    def _normalize_to_360(f: Callable):
        def decorator(self, start_date, rnday, constituent):
            return f(self, start_date, rnday, constituent) % 360.0

        return decorator

    @_manage_dates
    @_normalize_to_360
    def get_greenwich_factor(
        self, start_date: datetime, rnday: Union[float, timedelta], constituent: str
    ):
    ...
    def get_lunar_mean_longitude(self):
        return (
            ...
            + 0.549016532 * self.start_date_utc.hour
        )
    ...
    def get_solar_perigee(self):
        return (
            ...
            + 0.000001961 * self.start_date_utc.hour
        )
    ...
    def get_solar_mean_longitude(self):
        return (
            ...
            + 0.0410686387 * self.start_date_utc.hour
        )
    ...
    def all_constituents(self):
        ...
        elif isinstance(self.tidal_database, TPXO) or isinstance(
            self.tidal_database, FES2014
        ):
    ...
    def hour_middle(self):
        return self.start_date_utc.hour + (
            (self.end_date_utc - self.start_date_utc).total_seconds() / 3600 / 2
        )
    ...
    @property
    def DYR(self):
        return self.start_date_utc.year - 1900.0

    @property
    def DDAY(self):
        return (
            self.start_date_utc.timetuple().tm_yday
            + int((self.start_date_utc.year - 1901.0) / 4.0)
            - 1
        )
    ...
    @property
    def DT(self):
        return 180.0 + self.start_date_utc.hour * (360.0 / 24)
