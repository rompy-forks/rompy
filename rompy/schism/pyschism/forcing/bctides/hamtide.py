...
class HAMTIDE(TidalDataProvider):
    ...
    def get_elevation(self, constituent, vertices):
        logger.info("Querying HAMTIDE for elevation constituent " f"{constituent}.")
        amp = 0.01 * self._get_interpolation("elevation", "AMPL", constituent, vertices)
        phase = self._get_interpolation("elevation", "PHAS", constituent, vertices)
        return amp, phase

    def get_velocity(self, constituent, vertices):
        logger.info("Querying HAMTIDE for velocity constituent " f"{constituent}.")
        uamp = 0.01 * self._get_interpolation("velocity", "UAMP", constituent, vertices)
        uphase = self._get_interpolation("velocity", "UPHA", constituent, vertices)
        vamp = 0.01 * self._get_interpolation("velocity", "VAMP", constituent, vertices)
        vphase = self._get_interpolation("velocity", "VPHA", constituent, vertices)
    ...
    def _get_interpolation(self, phys_var, ncvar, constituent, vertices):
        ...
        xi = xi.flatten()
        yi = yi.flatten()
        zi = self._get_resource(phys_var, constituent)[ncvar][yidx, xidx].flatten()
        values = griddata(
